...
export async function generateMetadata({params}: Props) {
  ...
  const t = await getTranslations({locale, namespace: 'BlogTips.meta'});
  return {title: (t as any)('title'), description: (t as any)('description')};
}

export default async function BlogTips({params}: Props) {
  const {locale} = await params;
  setRequestLocale(locale);
  const t = await getTranslations('BlogTips');
    ...
    <div>
      ...
      <article className="py-16 bg-white">
        <div className="max-w-4xl mx-auto px-4">
          ...
          <div className="bg-gray-100 rounded-xl p-8 mt-12 border border-gray-200">
            <h3 className="text-2xl font-bold text-deep-blue mb-4">{t('conclusion.title')}</h3>
            <p className="text-gray-600 leading-relaxed mb-6">{t('conclusion.body')}</p>
